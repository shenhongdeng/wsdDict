...
'''
这是一个工具函数代码，写的都是一些小的工具
还涉及到，一些SQL数据的操作
'''

# 　UUID: 通用唯一标识符 ( Universally Unique Identifier ), 对于所有的UUID它可以保证在空间和时间上的唯一性. 它是通过MAC地址, 时间戳, 命名空间, 随机数, 伪随机数来保证生成ID的唯一性,
#  有着固定的大小( 128 bit ).  它的唯一性和一致性特点使得可以无需注册过程就能够产生一个新的UUID
import uuid
import requests
import pymysql
